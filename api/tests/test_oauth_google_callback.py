...
def test_callback_happy_path(monkeypatch):
    ...
    state_id = "abc"
    nonce = "nonce123"
    r.setex(f"oauth:state:{state_id}", 300, '{"return_to": "/app"}')
    ...
    assert res.headers["location"].startswith("/app?token=")
    assert res.cookies.get("token")
    assert r.get(f"oauth:state:{state_id}") is None

    db = TestingSession()
    ga = db.query(models.GoogleAccount).first()
    assert ga.sub == "sub1"
    assert ga.refresh_token_enc is not None
    db.close()
    teardown()


@respx.mock
def test_callback_state_invalid(monkeypatch):
    client, _, teardown, _ = setup(monkeypatch)
    res = client.get("/api/auth/google/callback?state=bad&code=X")
    assert res.status_code == 400
    teardown()


@respx.mock
def test_callback_token_endpoint_error(monkeypatch):
    client, r, teardown, _ = setup(monkeypatch)
    state_id = "id2"
    nonce = "n2"
    r.setex(f"oauth:state:{state_id}", 300, '{"return_to": "/"}')
    r.setex(f"oauth:pkce:{state_id}", 300, "ver")
    r.setex(f"oauth:nonce:{state_id}", 300, nonce)
    state = oauth_google.signer.sign(state_id).decode()

    respx.post("https://token").mock(return_value=httpx.Response(500))

    res = client.get(f"/api/auth/google/callback?state={state}&code=C")
    assert res.status_code == 400
    teardown()


@respx.mock
def test_callback_does_not_overwrite_refresh_token(monkeypatch):
    client, r, teardown, TestingSession = setup(monkeypatch)
    db = TestingSession()
